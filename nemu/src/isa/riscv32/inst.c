...
#define immB() do { *imm = SEXT((BITS(i, 31, 31) << 12) | (BITS(i, 30, 25) << 5) | (BITS(i, 11, 8) << 4) | (BITS(i, 7, 7) << 11), 13); } while (0) 
...
#define JUMP(dnpc, npc, rd, snpc) (dnpc) = (npc); R((rd)) = snpc
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltui  , I, R(rd) = (long long) src1 < (long long) imm);
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (word_t) src1 >> src2);
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = (word_t) src1 << src2);
  // INSTPAT("")
  // Done
