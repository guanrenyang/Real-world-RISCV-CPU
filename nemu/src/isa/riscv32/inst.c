...
#define JUMP(dnpc, npc, rd, snpc) (dnpc) = (npc); R((rd)) = snpc
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltui  , I, R(rd) = (int64_t) SEXT(src1, 8*sizeof(word_t)) < (int64_t) SEXT(imm, 8*sizeof(word_t)));
  ...
  INSTPAT("000000 0????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (word_t) src1 >> src2);
  INSTPAT("000000 0????? ????? 001 ????? 00100 11", slli   , I, R(rd) = (word_t) src1 << src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = src1 - src2);
  
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));  
  INSTPAT("010000 0????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int64_t) SEXT(src1, 8*sizeof(word_t)) >> src2);
  // INSTPAT("")
  // Done
