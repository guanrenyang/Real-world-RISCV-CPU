...
void ftrace(vaddr_t dnpc, vaddr_t pc, int rd, int rs1, int type, bool is_ecall){
  ...
  if (is_ecall) {
	Log("curr_func_name: %s", curr_func_name);
	Log("func_name: %s", func_name);
  }
...
#define JUMP(dnpc, npc, pc, rd, rs1, snpc, type) (dnpc) = (npc); R((rd)) = snpc; IFDEF(CONFIG_FTRACE, ftrace(dnpc, pc, rd, rs1, type, false))
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(0, s->pc);CSR(0x342)=0x00000008;CSR(0x341)=s->pc);
  INSTPAT("0011000 00010 00000 000 00000 11100 11", eret   , R, s->dnpc = CSR(0x341));
